...
def test_compute_pseudo_ratio_stats_by_language_source_bucket_counts_per_bucket():
    df = pd.DataFrame(
        [
            {"language": "pt", "source": "s1", "source_kind": SOURCE_KIND_PSEUDO_LABEL},
            {"language": "pt", "source": "s1", "source_kind": "human_label"},
            {"language": "pt", "source": "s2", "source_kind": SOURCE_KIND_PSEUDO_LABEL},
            {"language": "pt", "source": "s2", "source_kind": SOURCE_KIND_PSEUDO_LABEL},
            {"language": "en", "source": "s1", "source_kind": "human_label"},
        ]
    )

    stats = compute_pseudo_ratio_stats_by_language_source_bucket(df)

    assert stats == {
        "pt::s1": {"pseudo": 1, "human": 1},
        "pt::s2": {"pseudo": 2, "human": 0},
        "en::s1": {"pseudo": 0, "human": 1},
    }


def test_validate_pseudo_ratio_stats_by_language_source_bucket_raises_for_exceeding_bucket():
    cfg = SimpleNamespace(max_pseudo_ratio_per_language=0.5)
    stats_by_bucket = {
        "pt::s1": {"pseudo": 2, "human": 1},
        "pt::s2": {"pseudo": 1, "human": 3},
    }

    with pytest.raises(
        ValueError,
        match="pseudo-label ratio exceeds max_pseudo_ratio_per_language for language\\+source bucket: pt::s1",
    ):
        validate_pseudo_ratio_stats_by_language_source_bucket(stats_by_bucket, cfg)


def test_merge_filtered_pseudo_labels_caps_by_language_and_source_bucket():
    cfg = SimpleNamespace(confidence_threshold=0.8, max_pseudo_ratio_per_language=0.5)
    human_labels_df = pd.DataFrame(
        [
            {"image_path": "pt_s1_h0.png", "label_text": "humano 0", "language": "pt", "source_kind": "human_label", "source": "s1"},
            {"image_path": "pt_s2_h0.png", "label_text": "humano 1", "language": "pt", "source_kind": "human_label", "source": "s2"},
            {"image_path": "pt_s2_h1.png", "label_text": "humano 2", "language": "pt", "source_kind": "human_label", "source": "s2"},
            {"image_path": "pt_s2_h2.png", "label_text": "humano 3", "language": "pt", "source_kind": "human_label", "source": "s2"},
            {"image_path": "pt_s2_h3.png", "label_text": "humano 4", "language": "pt", "source_kind": "human_label", "source": "s2"},
            {"image_path": "pt_s2_h4.png", "label_text": "humano 5", "language": "pt", "source_kind": "human_label", "source": "s2"},
        ]
    )
    pseudo_candidates_df = pd.DataFrame(
        [
            {"image_path": "pt_s1_p0.png", "prediction_text": "pseudo s1 0", "confidence": 0.95, "language": "pt", "source": "s1"},
            {"image_path": "pt_s1_p1.png", "prediction_text": "pseudo s1 1", "confidence": 0.94, "language": "pt", "source": "s1"},
            {"image_path": "pt_s2_p0.png", "prediction_text": "pseudo s2 0", "confidence": 0.93, "language": "pt", "source": "s2"},
            {"image_path": "pt_s2_p1.png", "prediction_text": "pseudo s2 1", "confidence": 0.92, "language": "pt", "source": "s2"},
        ]
    )

    merged_df = merge_filtered_pseudo_labels(
        human_labels_df=human_labels_df,
        pseudo_candidates_df=pseudo_candidates_df,
        cfg=cfg,
    )

    pseudo_df = merged_df.loc[merged_df["source_kind"] == SOURCE_KIND_PSEUDO_LABEL]
    assert len(pseudo_df) == 3
    assert len(pseudo_df.loc[pseudo_df["source"] == "s1"]) == 1
    assert len(pseudo_df.loc[pseudo_df["source"] == "s2"]) == 2
